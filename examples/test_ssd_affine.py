...
# Batch size
train_batch_size = 1
CLS=73  # category label
cate = 'laptop'
DIR_BASE='affine_' + cate # +'_clean'  # savedir base
...
def main():
    ...
    for idx, [imgs, annotations, masks] in enumerate(data_loader):
        if len(image) >= 1000:    # if too much data, use first 4000
            break
        if idx % 1000 == 0:
            print(idx)
    ...
    # print([len(mask[0]) for mask in test_masks])
    ...
    #     origin_loss = get_loss(ssd, copy.deepcopy(torch.Tensor(train_pert_images[j:j+1]).cuda()), copy.deepcopy(y_target))
    ...
    patch = np.load("patches/aware/ssd/"+cate+"/patch_{}.npy".format(eps))
    ...
    for j in range(test_images.shape[0]):
        ...
        pert_images, new_boxes = attack.apply_no_patch(torch.Tensor(test_images[j:j+1]).cuda(), patch_external=torch.Tensor(patch).cuda(), gts_boxes=torch.Tensor(test_gts_boxes[j]).cuda(), corrupt_type=0, masks=test_masks[j])        
        # pert_images = np.transpose(pert_images, (0,3,1,2)).astype(float)
        # origin_loss = get_loss(ssd, copy.deepcopy(pert_images), copy.deepcopy(y_target))
        # origin_loss = get_loss(ssd, copy.deepcopy(torch.Tensor(pert_images).cuda()), copy.deepcopy(y_target))
        # original_loss_history = append_loss_history(original_loss_history, origin_loss, test_pert_images.shape[0])
        pert_images = pert_images.cpu().numpy()
        pert_predictions = ssd.predict(pert_images/255.)
        # pert_predictions = ssd.predict(x=pert_images.astype(np.float32)/255.)
        # pert_predictions = ssd.predict(x=pert_images[j:j+1]/255.)
        try:
            ...
            if count > 0:
                # plot_image_with_boxes(img=np.ascontiguousarray(pert_images[0], dtype=np.uint8), boxes=predictions_boxes, pred_cls=predictions_class, gt_boxes=test_labels['boxes'][j])
                # plot_image_with_boxes(img=np.ascontiguousarray(np.transpose(pert_images[0],(1,2,0)), dtype=np.uint8), boxes=predictions_boxes, pred_cls=predictions_class, gt_boxes=test_labels['boxes'][j])
                # plot_image_with_boxes(img=np.ascontiguousarray(np.transpose(pert_images[0],(1,2,0)), dtype=np.uint8), boxes=predictions_boxes, pred_cls=predictions_class, gt_boxes=new_boxes)
                iscrowd = torch.zeros(len(new_boxes))
                # g is the ground truth, is the prediction
                # but we need to change it to [start, length], instead of [start, end]
        ...
        # cuda float shape torch.Size([1, 3, 300, 300])
        # (1,3,300,300)
        patched_images, new_boxes = attack.apply_multi_patch(torch.Tensor(test_images[j:j+1]).cuda(), patch_external=torch.Tensor(patch).cuda(), gts_boxes=torch.Tensor(test_gts_boxes[j]).cuda(), masks=test_masks[j])
        # returnpatched_images = patched_images.permute(0,3,1,2).float().cuda()
        ...
        try:
            ...
            if count > 0:
                # plot_image_with_boxes(img=np.ascontiguousarray(np.transpose(patched_images[0],(1,2,0)), dtype=np.uint8), boxes=predictions_boxes, pred_cls=predictions_class, gt_boxes=new_boxes)
                ...
                # print("index=", j, predictions_boxes, new_boxes, ious)
    ...
    print("IoU 0.5 Acc: ", origin_iou/cnt, ",", patched_iou/cnt)
