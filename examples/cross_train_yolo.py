...
cocoCLS=CLS
cate = 'laptop'
DIR_BASE='cross_yolo_ssd'  # savedir base
...
def brightness(x, coords=None, severity=3):  #(w, h, c) # 3 224 224
...
def main():
    ...
    for idx, [imgs, annotations, mask] in enumerate(data_loader):
    ...
    for eps in [0.5]:
        ...
        # return an numpy arra

        # yolo5 = torch.hub.load('ultralytics/yolov5', 'yolov5s')  # or yolov5n - yolov5x6, custom
        ...
        for j in range(test_images.shape[0]):
            ...
            # clean change 1
            # origin_loss = get_loss(frcnn, copy.deepcopy(torch.Tensor(test_pert_images[j:j+1]).cuda()), copy.deepcopy(y_target))
            # original_loss_history = append_loss_history(original_loss_history, origin_loss, test_pert_images.shape[0])
            # yolo5 inference
            
            results = yolo5(torch.Tensor(test_pert_images[j:j+1]/255.))[0]
            pert_predictions = {}
            ...
            # pert_predictions['boxes'] = results[:, :4]
            # pert_predictions['labels'] = results[:, -1].astype(np.int)
            # pert_predictions['scores'] = results[:, -2]
            # print(pert_predictions)
            try:
                # predictions_class, predictions_boxes, predictions_scores, count = extract_predictions(pert_predictions, name="Perturbed", thresh=0.0, cls=CLS)
                # if count > 0:
                #     # json results
            ...
            # loss = get_loss(frcnn, copy.deepcopy(patched_images), copy.deepcopy(y_target))
            # loss_history = append_loss_history(loss_history, loss, test_images.shape[0])
            # patched_images = patched_images[0].permute(1,2,0).cpu().numpy()
            results = yolo5(patched_images.cpu()/255.)[0]
            patch_predictions = {}
            # patch_predictions['boxes'] = results[:, :4]
            # patch_predictions['labels'] = results[:, -1].astype(np.int)
            # patch_predictions['scores'] = results[:, -2]
            ...
            try:
                # predictions_class, predictions_boxes, predictions_scores, count = extract_predictions(patch_predictions, name="Patched", thresh=0.0, cls=CLS)    
                # if count > 0:
                #     # json results
                #     for kk in range(len(predictions_boxes)):
                #         res = dict()
                ...
                # print(predictions_class, predictions_scores)
                if count > 0:
                    patched_images = np.transpose(patched_images[0].cpu().numpy(),(1,2,0))
                    # plot_image_with_boxes(img=np.ascontiguousarray(patched_images), boxes=predictions_boxes, pred_cls=predictions_class, gt_boxes=test_labels['boxes'][j])
