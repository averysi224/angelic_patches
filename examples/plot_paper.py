...
cate = 'person'
DIR_BASE='examples/tmp_patch'  # savedir base
...
def main():
    ...
    for eps in [0.5]:
        ...
        for j in range(test_images.shape[0]):
            ...
            img=np.ascontiguousarray(np.transpose(test_images[j],(1,2,0)), dtype=np.uint8)
            img = img[:, :, ::-1]
            plt.imshow(img.astype(np.uint8), interpolation="nearest")
            plt.savefig(DIR+"/clean_test_image_{}.png".format(j))

            img=np.ascontiguousarray(np.transpose(test_pert_images[j],(1,2,0)), dtype=np.uint8)
            img = img[:, :, ::-1]
            plt.imshow(img.astype(np.uint8), interpolation="nearest")
            plt.savefig(DIR+"/pert_test_image_{}.png".format(j))

            patched_clean = attack.apply_no_corruption(torch.Tensor(test_images[j:j+1]).cuda(), patch_external=torch.Tensor(patch).cuda(), gts_boxes=torch.Tensor(test_gts_boxes[j]).cuda())
            patched_clean = patched_clean.cpu().numpy()
            img=np.ascontiguousarray(np.transpose(patched_clean[0],(1,2,0)), dtype=np.uint8)
            img = img[:, :, ::-1]
            plt.imshow(img.astype(np.uint8), interpolation="nearest")
            ...
            img=np.ascontiguousarray(np.transpose(patched_images[0],(1,2,0)), dtype=np.uint8)
            img = img[:, :, ::-1]
            plt.imshow(img.astype(np.uint8), interpolation="nearest")
            plt.savefig(DIR+"/cpatched_test_image_{}.png".format(j))
