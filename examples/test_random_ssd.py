...
cate = 'laptop'
DIR_BASE='test_clear_' #+ cate  # savedir base
...
def main():
    ...
    for j in range(test_images.shape[0]):
        ...
        # (1, 3, 300, 300)
        # pert_images = test_images[j:j+1]
        pert_images = attack.apply_no_patch(torch.Tensor(test_images[j:j+1]).cuda(), patch_external=torch.Tensor(patch).cuda(), gts_boxes=torch.Tensor(test_gts_boxes[j]).cuda(), masks=test_masks[j])        
        # torch.Size([1, 300, 300, 3]), 255
        ...
        pert_images = pert_images.permute(0,3,1,2).cpu().numpy()
        ...
        patched_images, rand_n = attack.apply_multi_patch(torch.Tensor(test_images[j:j+1]).cuda(), patch_external=torch.Tensor(patch).cuda(), gts_boxes=torch.Tensor(test_gts_boxes[j]).cuda(), masks=test_masks[j], corrupt_type=0)
        # torch.Size([1, 300, 300, 3])
        # patched_images = patched_images.permute(0,3,1,2).float().cuda()
        patched_cnt += rand_n
        ...
        ###
        patched_images = patched_images.permute(0,3,1,2).cpu().numpy()
    ...
    # print("0.5 IoU high Conf Acc: ", origin_iou/patched_cnt, ",", patched_iou/patched_cnt)
    
    print("0.5 IoU high Conf Acc: partial ", origin_iou/cnt, ",", patched_iou/cnt, ",", patched_iou/patched_cnt)
